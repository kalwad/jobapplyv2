...
Spec version: 1.4
Repository revision: M02-W07 KI-0058 ninth correction writer pass on independently blocked eighth-correction commit 1a63145846e44f60903e48402c0c812d02581dd5 / tree ce945ff89af363e8da2bd28ba60113b5e6d2a75d / parent 3133a9348193a03759140dc2a32c35223be9717d
Last updated: 2026-08-21T15:51:20-04:00
...
- State: M02-W07 (scaffold the real MV3 feasibility extension) is IN_PROGRESS. This ninth narrow KI-0058 correction writer pass began at independently blocked eighth-correction commit `1a63145846e44f60903e48402c0c812d02581dd5` / tree `ce945ff89af363e8da2bd28ba60113b5e6d2a75d` / parent `3133a9348193a03759140dc2a32c35223be9717d` after verdict `SOL_BLOCKED_FINAL_M02_W07_EIGHTH_CORRECTION_VERIFICATION`. That fresh verifier cleared the eighth correction's exact direct local closure semantics and found one remaining pre-existing hole: an immediately called conditional expression whose selected arrow/function result captures the tracked option object had no direct target, so both statement relevance and call-time processing preserved stale certainty. This pass corrects only that immediate non-direct callee-result boundary while preserving inert uncalled bodies, exact direct identifier summaries, and the fifth-through-eighth locks. It is executed by GPT-5.6 Sol Ultra — implementation provenance only, not independent verification. No package is READY, M02 remains IN_PROGRESS, and M00 and M01 remain ACCEPTED. M02-W06 remains VERIFIED, not ACCEPTED, at exact content commit `f4ffcf7064fe0f077b948690cebbee385fe190fb` / tree `6fd4219460a7659b21576f2ca20b19b744f3bbf9` after `SOL_CLEAR_FINAL_M02_W06_CONTENT` on 2026-08-12.
...
- Implementation: the WXT 0.20.27 substrate remains minimal and performs no product action. Its 63 extension tests lock the one probe/ACK surface and the generated manifest. The 72-test / 23-file Playwright suite includes 13 W07 tests against fresh persistent bundled-Chromium contexts: a test-only invalid-ACK build re-exports the actual production content entrypoint and proves the marker is ACK-caused; a pristine no-extension baseline is compared with activation and reload through a generic snapshot of every live form control, validation state, status/error region, storage, route, flow, honeypot, and receipt state; extension-origin messaging proves only the literal probe receives the literal ACK and rejects multiple unrelated command values; and a pre-navigation Chrome trace plus exact page observer bounds WXT's unavoidable non-sensitive lifecycle CustomEvent while the full message list remains empty. PORT-SRC-008 uses the repository-pinned TypeScript compiler without executing repository source and now derives its operational semantics from one reviewed pinned Node catalog (Node 24.18.0 / TypeScript 6.0.3 / @types/node 24.13.3, fail-closed pin and field validation, exact-completeness declaration oracle with a nested-surface drift rule) across five layers: bounded JavaScript primitive constants; pure trusted `node:path` join/resolve abstract results with posix/win32 flavor truth including the official submodule specifiers; expression-resolved local option-object state with real loop back-edge fixed points across every loop form, body-mutation-guarded exact counts, a shared exact-replay budget, modeled delete-to-absent, and reference-preserving merges; role-exact child-process and `process.execve` invocation semantics matching empirically verified pinned runtime shell truth (exec-family default shell and empty-selector exception, truthy-selector spawn/execFile family, forced-disabled fork, inert argv, spread-surviving constant argv prefixes, `bash`/`sh` basename wrapper tuples with exact `-c`/`-lc` parity while `bash -client`/`sh -client` pass); and closed module/member provenance (static ESM forms including `default as` and the namespace `default` member, bounded `.cts` require, the ambient global `process`, direct `await import()` and `process.getBuiltinModule` re-entry) that terminates at arbitrary or augmented members. The fifth correction makes proved violation facts monotonic across the bounded analysis budget — statements that never reference the tracked option object are skipped, sink-argument refinement runs on clones, an exhausted analysis unions refined with pre-refinement at-sink states so a proved reachable violation is never erased, and exhaustion itself fails closed as an explicit `analysis-budget` finding instead of a silently completed clean analysis — and adds a fail-closed operational-constructor catalog classification (`filesystem-path-options`) that reclassifies `fs.Utf8Stream` with `dest` as its reviewed path-bearing option so constructor sinks resolve only through trusted catalog provenance. The sixth correction closes the two reviewer-reproduced tracked-object fail-open families with one shared alias abstraction: a strict same-object proof registers a definite alias for every runtime-no-op alias form the event scan accepts (parenthesized/`as`/`satisfies`/type-assertion/non-null wrappers, proved-taken `||`/`&&`/`??` operands, comma results, and same-object conditionals), so mutations through registered aliases are exactly as visible as mutations through the tracked identifier; ambiguous initializers that may reference the tracked object register as may-aliases whose writes, deletes, and escapes only degrade certainty to UNKNOWN and can never establish or preserve a proof; a supported-looking alias-value read whose declaration is not registered escapes conservatively instead of being treated as harmless; and a shorthand-property store (`{ options }`) escapes by value symbol exactly like its longhand equivalent at the store's evaluation point, with the statement skip extended to see shorthand value symbols and may-alias references. The seventh correction resolves direct local `FunctionDeclaration` calls by TypeScript symbol identity regardless of whether declaration text precedes or follows the call; declarations are no-ops in statement flow, while exact straight-line zero-parameter tracked-property assignments/deletes are interpreted at the call site using the existing state machinery. The eighth correction extends that same symbol-resolved direct-call abstraction to stable local `const` arrows and function expressions initialized before the call, including concise assignment bodies, without executing uncalled declarations. Exact bodies reuse the existing tracked write/delete evaluator at call time; parameter/default, async/generator, mutable/reassigned, aliased, recursive/cyclic, over-depth, unsupported-control-flow/call, and nested captured-closure shapes conservatively invalidate to UNKNOWN when they may touch tracked state. The ninth correction leaves that exact direct-identifier path unchanged and adds one structurally bounded immediate-callee-result relevance helper for non-direct calls. It unwraps parentheses/`as`/`satisfies`/type assertions/non-null, selects only a statically reachable conditional result when known and either possible result when unknown, takes only the callable result of comma expressions, and conservatively covers `&&`/`||`/`??` result forms. If a possible arrow/function result's runtime nodes may touch tracked state, both the statement filter and call processor agree that the call is relevant, and the state becomes UNKNOWN after normal callee/argument evaluation; unrelated immediate callables do not invalidate. Deferred declarations remain inert until called, direct arrow/function IIFEs retain the eighth correction's conservative behavior, and exact direct identifiers retain their summaries. The active-symbol set and depth-16 bound remain intact. Unsupported semantics stay UNKNOWN and never prove a violation; analyzer infrastructure failures remain fail-closed. Reviewed suppressive-direction boundaries and the parked future-hardening families are pinned in docs/KNOWN_ISSUES.md § KI-0058 (KI-0006 TypeScript surface; the Rust/native surface stays DEFERRED to M17). Runtime request observation covers only traffic after listener attachment; static source/shipped-byte checks cover the launch-window absence of egress primitives. REQ-FORM-020 remains SCAFFOLD_ONLY / NOT_YET_APPLICABLE. Full evidence and exact mutation kills sit in docs/TEST_EVIDENCE.md § M02-W07; exact-SHA three-OS hosted evidence is pending this writer pass.
- Preserved predecessor evidence: M02-W06 remains VERIFIED with its hosted three-OS governance proof (run `31645800902` at governance commit `e884917a2f82c392e99d340018c6da6919641bf0`) and the preserved owner-bundle verification chain recorded in docs/TEST_EVIDENCE.md § M02-W06. The sanitized owner-holdout commitment (manifest digest `sha256:a10ecd8f5eb4f930b056e6ed375627ef42055fece1c5ffb86ef198b1cebe5a79`), `benchmarks/holdout-manifests/status.v1.json` lifecycle markers, `model/model-lock.json`, `prompts/registry.yaml`, generated benchmark contracts, public corpus truth, and all four critical-gate reports remain untouched by M02-W07. KI-0046 through KI-0057 remain FIXED; KI-0001 is FIXED by this package's real build task, build suite, and evidence; KI-0006 records its TypeScript rule surface implemented here, while HIGH / IN_PROGRESS KI-0058 tracks independent sufficiency verification of the corrected semantics; KI-0006 itself stays DEFERRED for the untouched M17 Rust surface. KI-0003, KI-0022, KI-0026, and KI-0027 remain DEFERRED with their named future owners.
...
- Blocker posture: KI-0058 remains the only substantive implementation blocker. The fresh eighth-correction verifier cleared direct function declarations and stable direct local arrow/function-expression calls and blocked only on immediate non-direct callable-result relevance; this ninth narrow writer pass closes that stale-certainty boundary with conservative call-time UNKNOWN, but does not govern or close KI-0058. KI-0059 through KI-0062 remain independently clear; their implementation/test surfaces were not reopened, and they remain HIGH / IN_PROGRESS solely pending the next completely fresh verifier and W07 governance. W07 remains IN_PROGRESS, W08 remains NOT_STARTED, and no package is READY.
...
| `M02-W07` | IN_PROGRESS | — | — | Scaffold the real MV3 feasibility extension |
...
- ID: NONE
- Reason: M02-W07 is IN_PROGRESS (single-IN_PROGRESS rule; READY cannot coexist with IN_PROGRESS). M02-W08 becomes READY only after a completely fresh independent verifier confirms the exact M02-W07 content and M02-W07 is marked VERIFIED.
- Required reading for the next transition: docs/MASTER_IMPLEMENTATION_SPEC.md § M02-W07, apps/extension/README.md, and docs/TEST_EVIDENCE.md § M02-W07.
