...
Spec version: 1.4
Repository revision: M02-W07 KI-0058 second correction writer pass on independently blocked first-correction commit f4ee49e6f056f2cc15257a6d8a0cbc43de7b1941 / tree 04721b7ea4fd0c2eec2506e507ea3e37c743e2ba / parent 6cf4d4b2860c054868dbe22600a0f6455cc7b60a
Last updated: 2026-08-14T00:42:44-04:00
...
- State: M02-W07 (scaffold the real MV3 feasibility extension) is IN_PROGRESS. This second narrow correction writer pass began at independently blocked first-correction commit `f4ee49e6f056f2cc15257a6d8a0cbc43de7b1941` / tree `04721b7ea4fd0c2eec2506e507ea3e37c743e2ba` / parent `6cf4d4b2860c054868dbe22600a0f6455cc7b60a` after verdict `SOL_BLOCKED_FINAL_M02_W07_CORRECTED_VERIFICATION`. That verifier independently cleared the KI-0059, KI-0060, KI-0061, and KI-0062 implementation surfaces and identified KI-0058 as the only remaining substantive blocker. No package is READY, M02 remains IN_PROGRESS, and M00 and M01 remain ACCEPTED. M02-W06 remains VERIFIED, not ACCEPTED, at exact content commit `f4ffcf7064fe0f077b948690cebbee385fe190fb` / tree `6fd4219460a7659b21576f2ca20b19b744f3bbf9` after `SOL_CLEAR_FINAL_M02_W06_CONTENT` on 2026-08-12.
...
- Implementation: the WXT 0.20.27 substrate remains minimal and performs no product action. Its 63 extension tests lock the one probe/ACK surface and the generated manifest. The 72-test / 23-file Playwright suite includes 13 W07 tests against fresh persistent bundled-Chromium contexts: a test-only invalid-ACK build re-exports the actual production content entrypoint and proves the marker is ACK-caused; a pristine no-extension baseline is compared with activation and reload through a generic snapshot of every live form control, validation state, status/error region, storage, route, flow, honeypot, and receipt state; extension-origin messaging proves only the literal probe receives the literal ACK and rejects multiple unrelated command values; and a pre-navigation Chrome trace plus exact page observer bounds WXT's unavoidable non-sensitive lifecycle CustomEvent while the full message list remains empty. PORT-SRC-008 now uses the repository-pinned TypeScript compiler without executing repository source and separates bounded primitive-constant evaluation from policy relevance. It resolves constant object-literal property names/values and rejects the proved fact `shell = true`. For path/wrapper facts, anchored complete-value checks cover initializers, property/array/return/yield/arrow/export/JSX values, plain assignment right-hand sides, and every call/new argument; embedded fragments require a closed Node module/operation/argument-position signature. Provenance covers non-type ESM imports and bounded, non-shadowed `.cts` import-equals/const/inline/destructured `require(<literal>)` forms, following const callee/namespace aliases. The finite signatures inspect child-process command/static-argv and five path-like option fields, allowlisted filesystem path operands but never payload/encoding/callback positions, path operands/format path fields, and only `process.chdir`, `loadEnvFile`, and `dlopen` path positions. Wrapper matching uses exact whitespace-delimited `bash -c`, `bash -lc`, or `sh -c` tokens, so `bash -client` is not a violation. Comments, JSDoc, type-only syntax, standalone descriptions, descriptions whose complete value is not prohibited, unrelated computed properties, and UNKNOWN dynamic expressions do not violate without a proved policy fact (KI-0006 TypeScript surface; the Rust surface stays DEFERRED to M17). Runtime request observation covers only traffic after listener attachment; static source/shipped-byte checks cover the launch-window absence of egress primitives. REQ-FORM-020 remains SCAFFOLD_ONLY / NOT_YET_APPLICABLE. Full evidence and exact mutation kills sit in docs/TEST_EVIDENCE.md § M02-W07; exact-SHA three-OS hosted evidence is pending this writer pass.
...
- Blocker posture: KI-0058 remains the only substantive implementation blocker after the independently blocked first-correction commit `f4ee49e6f056f2cc15257a6d8a0cbc43de7b1941` / tree `04721b7ea4fd0c2eec2506e507ea3e37c743e2ba`. This second narrow writer pass corrects its remaining semantic boundary but does not govern or close it. KI-0059 through KI-0062 were independently cleared on that first-correction content; their implementation/test surfaces were not reopened, and they remain HIGH / IN_PROGRESS solely pending the next completely fresh verifier and W07 governance. W07 remains IN_PROGRESS, W08 remains NOT_STARTED, and no package is READY.
