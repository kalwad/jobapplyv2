...
Spec version: 1.4
Repository revision: M02-W07 KI-0058 third correction writer pass on independently blocked second-correction commit 2b85c9b1e1f80cd41334752e14b259dc61151058 / tree 7c97431ef520b3ebefe0c0a0a81c147c830d7351 / parent f4ee49e6f056f2cc15257a6d8a0cbc43de7b1941
Last updated: 2026-08-14T05:31:26-04:00
...
- State: M02-W07 (scaffold the real MV3 feasibility extension) is IN_PROGRESS. This third narrow KI-0058 correction writer pass began at independently blocked second-correction commit `2b85c9b1e1f80cd41334752e14b259dc61151058` / tree `7c97431ef520b3ebefe0c0a0a81c147c830d7351` / parent `f4ee49e6f056f2cc15257a6d8a0cbc43de7b1941` after verdict `SOL_BLOCKED_FINAL_M02_W07_KI0058_VERIFICATION`. The verifier again left the independently clear KI-0059, KI-0060, KI-0061, and KI-0062 implementation surfaces closed and identified only fresh KI-0058 semantic gaps. No package is READY, M02 remains IN_PROGRESS, and M00 and M01 remain ACCEPTED. M02-W06 remains VERIFIED, not ACCEPTED, at exact content commit `f4ffcf7064fe0f077b948690cebbee385fe190fb` / tree `6fd4219460a7659b21576f2ca20b19b744f3bbf9` after `SOL_CLEAR_FINAL_M02_W06_CONTENT` on 2026-08-12.
...
- Implementation: the WXT 0.20.27 substrate remains minimal and performs no product action. Its 63 extension tests lock the one probe/ACK surface and the generated manifest. The 72-test / 23-file Playwright suite includes 13 W07 tests against fresh persistent bundled-Chromium contexts: a test-only invalid-ACK build re-exports the actual production content entrypoint and proves the marker is ACK-caused; a pristine no-extension baseline is compared with activation and reload through a generic snapshot of every live form control, validation state, status/error region, storage, route, flow, honeypot, and receipt state; extension-origin messaging proves only the literal probe receives the literal ACK and rejects multiple unrelated command values; and a pre-navigation Chrome trace plus exact page observer bounds WXT's unavoidable non-sensitive lifecycle CustomEvent while the full message list remains empty. PORT-SRC-008 uses the repository-pinned TypeScript compiler without executing repository source and now separates four semantic layers: bounded JavaScript primitive constants; pure trusted `node:path.join`/`resolve` abstract results; bounded point-in-time local state for reviewed child-process option properties; and closed operational sink classification. JavaScript unary coercion, ordered direct/computed writes, final overwrite state, supported possible branches, per-link optional-chain reachability, sink argument order, and const-aliased path composition are modeled. Unsupported types stay UNKNOWN; alias escape, reassignment, unsupported mutation/control flow, and ambiguous nested abrupt flow invalidate retained state rather than preserving stale facts. Path/wrapper findings require proved non-type ESM or bounded non-shadowed `.cts` CommonJS provenance and a reviewed filesystem/process/child-process argument position. Arbitrary exports, initializers, calls, console/stdout values, unused path results, and filesystem payload positions are data, not operational sinks; variable names supply no provenance. Composed `bash` plus exact `-c`/`-lc` or `sh` plus `-c` still fails, while `bash -client` passes. Malformed source and helper/protocol failures remain fail-closed (KI-0006 TypeScript surface; the Rust/native surface stays DEFERRED to M17). Runtime request observation covers only traffic after listener attachment; static source/shipped-byte checks cover the launch-window absence of egress primitives. REQ-FORM-020 remains SCAFFOLD_ONLY / NOT_YET_APPLICABLE. Full evidence and exact mutation kills sit in docs/TEST_EVIDENCE.md § M02-W07; exact-SHA three-OS hosted evidence is pending this writer pass.
