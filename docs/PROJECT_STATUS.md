...
Spec version: 1.4
Repository revision: M02-W08 visibility correction in progress from required application-root correction commit bc32eb21ea881dbdd46b8e40d9b07b0e653020aa / tree 4ef959a569c546b566c09557f26d27123197f88d
Last updated: 2026-08-22T13:04:30-04:00
...
- State: M02-W08 (implement semantic field identity and per-frame scanner) is IN_PROGRESS. The narrow visibility correction writer began from the required clean synchronized application-root correction commit `bc32eb21ea881dbdd46b8e40d9b07b0e653020aa` / tree `4ef959a569c546b566c09557f26d27123197f88d`, whose parent is the required implementation commit `b7b7a157aa10e8f2dd0b9f15a06c46e7327b33c3`. No package is READY, M02 remains IN_PROGRESS, and M00 and M01 remain ACCEPTED. M02-W07 remains VERIFIED, not ACCEPTED, at exact independently cleared content commit `ce74ef49e142bb7c2cab608cd0c800d312bd2217` / tree `9412c5b71437ee562534a1fb92e80ab50ffc333d` after verdict `SOL_CLEAR_FINAL_M02_W07_NINTH_CORRECTION_CONTENT` and exact-SHA hosted run `32522233170`.
- Application-root correction: the first fresh W08 verifier stopped because one qualifying native form nested in semantic `main` was widened to that ancestor and admitted unrelated sibling controls. The correction returns the unique native or `role=form` candidate itself after no stronger explicit application root wins; explicit-root precedence, no-form semantic-main fallback, and multi-form ambiguity remain unchanged, and six finite bundled-Chromium regressions (C1–C6) lock those boundaries. The second fresh verifier independently confirmed this root correction clear at commit `bc32eb21ea881dbdd46b8e40d9b07b0e653020aa` before stopping on the visibility check below; root selection and precedence were not changed again.
- Visibility correction: the second fresh W08 verifier (`SOL_BLOCKED_FINAL_M02_W08_CORRECTION_VERIFICATION`) reproduced in bundled Chromium that `visible` depended on the current viewport: an ordinary in-flow required field below the initial fold reported `visible=false` at scroll position zero, and after `scrollIntoView` on the unchanged document the below-fold field became `true` while the above-fold field became `false`. The correction redefines W08 visibility as a rendered, non-concealed box inside the document's user-reachable scrollable area: the bounding box is normalized with the window scroll offsets and compared against the document's scroll dimensions, and boxes anchored to a fixed-position control or ancestor are judged against the viewport they are pinned to. Hidden, `aria-hidden`, and `inert` ancestry, `display:none`, `visibility:hidden`/`collapse`, opacity zero, zero-sized boxes, and deliberately displaced negative-coordinate off-canvas controls remain invisible. Five finite bundled-Chromium regressions (V1–V7) lock below-fold visibility, scroll invariance of the complete descriptor set, off-canvas rejection, the concealment families, and fixed-anchored controls; V7 reproduces and closes the bounded reviewer's in-scope finding that a control inside an off-viewport `position:fixed` container drifted with window scroll. Root precedence, scanner protocol, semantic identity, frame isolation, re-resolution, and every W09/W10/W11 surface remain untouched; fresh independent W08 verification is still required.
