...
class PlatformCertificationInputV2EvidenceInventoryItem(ContractModel):
    ...
    evidence_record_ref: CommonStableIdV1StableId

class PlatformCertificationInputV2(ContractModel):
    "The evidence inventory a future owner decision would consume before a platform support tier could be published. It is an input, never a decision: a certified proposal is evaluated against the canonical tier policy and a kind-to-record inventory, never against a self-declared required set. M01-W07 certifies no platform and changes no critical-gate state."
    ...
    evidence_inventory: Annotated[Annotated[list[PlatformCertificationInputV2EvidenceInventoryItem], MinLen(0), MaxLen(16)], Field(description="Strictly sorted evidence-kind bindings used to derive both the present-kind and evidence-reference inventories.")]
